...
{
    public class GoflywayFmt : BaseFmt
    {
        public static ProfileItem? Resolve(string str, out string msg)
        {
            msg = ResUI.ConfigurationFormatIncorrect;

            var item = ResolveGoflyway(str) ?? ResolveGoflywayNew(str);
            if (item == null)
            {
                return null;
            }
            ...
            item.ConfigType = EConfigType.Goflyway;
        ...
        public static string? ToUri(ProfileItem? item)
        {
            ...
            return ToUri(EConfigType.Goflyway, item.Address, item.Port, pw, null, remark);
        }

        private static ProfileItem? ResolveGoflyway(string result)
        {
            ProfileItem item = new()
            {
                ConfigType = EConfigType.Goflyway
            };
            result = result[Global.ProtocolShares[EConfigType.Goflyway].Length..];
            //remark
        ...
        private static ProfileItem? ResolveGoflywayNew(string result)
        {
